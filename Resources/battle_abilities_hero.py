...
def find_single_enemy(b, TileNum):
    unit = None
    for army in game_obj.game_armies:
        ...
        for effect in unit.effects:
            if effect.name == effect_name:
                ...
                for eff_card in b.queue:
                    if eff_card.position == unit.position and eff_card.obj_type == "Effect" and eff_card.title == effect_name:
...
# Scripts
def increase_morale(b, TileNum, hero):
    unit = find_single_ally(b, TileNum)
    print("Before: unit.morale - " + str(unit.morale))
    if unit.morale + 0.3 > unit.leadership:
        unit.morale = float(unit.leadership)
    else:
        unit.morale = unit.morale + 0.3
    print("After: unit.morale - " + str(unit.morale))


def advance_initiative(b, TileNum, hero):
    print("advance_initiative: " + hero.hero_class + " " + hero.name)
    initiative = 1.0
    MP_bonus = 0
    MP_addition_list = []
    ...
    artifact_bonus("Direct order", "Tactics", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
    ...
    hero.initiative = float(initiative)


def bless_spell(b, TileNum, hero):
    initiative = 1.0
    unit = find_single_ally(b, TileNum)
    ...
    artifact_bonus("Bless", "Devine magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
    ...
    hero.mana_reserve -= int(ability_catalog.ability_cat[b.selected_ability].mana_cost + mana_bonus)
    hero.initiative = float(initiative)


def haste_spell(b, TileNum, hero):
    initiative = 1.0
    unit = find_single_ally(b, TileNum)
    ...
    artifact_bonus("Haste", "Devine magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
...
def healing_spell(b, TileNum, hero):
    initiative = 1.0
    unit = find_single_ally(b, TileNum)
    ...
    hero.mana_reserve -= int(ability_catalog.ability_cat[b.selected_ability].mana_cost + mana_bonus)
    print("hero.initiative - " + str(hero.initiative) + ", time_act - " + str(b.queue[0].time_act) +
          ", initiative - " + str(initiative))
    hero.initiative = float(initiative)


def devine_protection_spell(b, TileNum, hero):
    initiative = 1.0
    unit = find_single_ally(b, TileNum)
    ...
    artifact_bonus("Devine protection", "Devine magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
...
def devine_strength_spell(b, TileNum, hero):
    initiative = 1.0
    unit = find_single_ally(b, TileNum)
    ...
    artifact_bonus("Devine strength", "Devine magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
...
def inspiration_spell(b, TileNum, hero):
    initiative = 1.0
    unit = find_single_ally(b, TileNum)
    ...
    artifact_bonus("Inspiration", "Devine magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
...
def missile_shielding_spell(b, TileNum, hero):
    initiative = 1.0
    unit = find_single_ally(b, TileNum)
    ...
    artifact_bonus("Missile shielding", "Devine magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
...
def fire_arrows_spell(b, TileNum, hero):
    # Reset damage counter
    b.dealt_damage = 0
    b.killed_creatures = 0

    initiative = 1.0
    unit = find_single_enemy(b, TileNum)
    ...
    artifact_bonus("Fire arrows", "Elemental magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
    ...
    print("hero.magic_power - " + str(hero.magic_power) + ", final_MP - " + str(final_MP))
...
def lightning_bolts_spell(b, TileNum, hero):
    # Reset damage counter
    b.dealt_damage = 0
    b.killed_creatures = 0
    ...
    artifact_bonus("Lightning bolts", "Elemental magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
...
def hail_spell(b, TileNum, hero):
    ...
    artifact_bonus("Hail", "Elemental magic", hero, MP_addition_list, initiative_reduction, mana_cost_subtraction)
